(function () {
    ...
    const getSelectedValues = (form, selector) =>
        Array.from(form.querySelectorAll(`${selector} option:checked`)).map(option => option.value);     

    function onSubmit(pristine, form) {
        let valid = pristine.validate();

        if (valid) {
            let platformId = getInputValue(form, 'input[name="platform-id"]');
            let formData = {
                ...
                type: getSelectedValues(form, 'select[name="platformTypes[]"]'),
                desc: getInputValue(form, 'textarea[name="platform-desc"]'),
                cluster: getSelectedValues(form, 'select[name="eaClusters[]"]'),
            ...
            axios.put(`/digital-platforms/${platformId}`, formData)
    ...
    $('input[name="owner"]').on('click', toggleDropdowns);     
