(function () {
    ...
    function onSubmit(form) {
        if (validateForm(form)) {
            ...
            const publishSurveyIdElement = document.querySelector('meta[name="publish-survey-id"]');
            if (!publishSurveyIdElement) {
                console.error("Meta tag with publish survey ID not found.");
                alert("An error occurred: Survey ID not found.");
                return;
            }
            const publishSurveyId = publishSurveyIdElement.content;

            axios
                .post(`/publish-surveys/${publishSurveyId}/select-team/add`, formData)
    ...
    function onRemoveClick(event) {
        ...
        const publishSurveyId = document.querySelector('meta[name="publish-survey-id"]').content;

        if (confirm("Are you sure you want to remove this team member?")) {
            axios
                .delete(`/publish-surveys/${publishSurveyId}/select-team/remove`, {
                    data: { user_id: userId },
                })
