...
@section('subcontent')
    {{-- {{dd($publish_survey->toArray());}} --}}
    <div class="mt-8 flex items-center">
        <h2 class="intro-y mr-auto text-lg font-medium">Select Digital Platform</h2>
    </div>
    <!-- BEGIN: Wizard Layout -->
    ...
    <div class="intro-y col-span-12 mt-6 ">
        ...
        >
            <span>
                Enter the digital platform name or a keyword in the search bar below and press 'Enter' to 
                find matching platforms. Select your desired platform by clicking its checkbox, then 
    ...
    <!-- BEGIN: Digital Platform List -->
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <form>
            @csrf
            <input type="hidden" name="survey-id" value="{{ $publish_survey->id }}">
            <input type="hidden" name="selected-platform-id" value="{{ session('selected_platform_id') }}">
                    ...
                    @foreach ($platforms as $platform)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td
                                ...
                                data-platform-title="{{ $platform->platform_name }}"
                            >
            ...
            <div class="mt-6 mb-6 flex flex-col justify-end gap-2 md:flex-row">
                <x-base.button type="button" as="a" href="{{ route('publish-surveys.selectSchema', $publish_survey->id) }}" class="w-full border-slate-300 py-3 text-slate-500 dark:border-darkmode-400 md:w-52">
                    Previous
                </x-base.button>
    ...
    <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
        <x-base.pagination class="w-full sm:mr-auto sm:w-auto">
            ...
            @else
                <x-base.pagination.link href="{{ $platforms->url(1) }}">
                    <x-base.lucide class="h-4 w-4" icon="ChevronsLeft" />
                </x-base.pagination.link>
            @endif
            ...
            @else
                <x-base.pagination.link href="{{ $platforms->previousPageUrl() }}">
                    <x-base.lucide class="h-4 w-4" icon="ChevronLeft" />
                </x-base.pagination.link>
            @endif
        
            <!-- Page Numbers -->
            @foreach ($platforms->getUrlRange(max(1, $platforms->currentPage() - 1), min($platforms->lastPage(), $platforms->currentPage() + 1)) as $page => $url)
            ...
            @if ($platforms->hasMorePages())
                <x-base.pagination.link href="{{ $platforms->nextPageUrl() }}">
            ...
            <!-- Last Page Link -->
            @if ($platforms->hasMorePages())
                <x-base.pagination.link href="{{ $platforms->url($platforms->lastPage()) }}">
        ...
        <form method="GET" action="{{ route('publish-surveys.selectApp', $publish_survey->id) }}" class="mt-3 w-20 sm:mt-0">
...
@pushOnce('scripts')
    @vite('resources/js/pages/selectApp.js')
@endPushOnce