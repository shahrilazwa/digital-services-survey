...
@section('subcontent')
    ...
    <div class="mt-8 flex items-center">
        <h2 class="intro-y mr-auto text-lg font-medium">Select Digital Service</h2>
    </div>
    ...
    <div class="intro-y col-span-12 mt-6 ">
        ...
        >
            <span>
                Enter the digital service title or a keyword in the search bar below and press 'Enter' to 
                find matching service. Select your desired service by clicking its checkbox, then 
    ...
    <!-- BEGIN: Digital Service List -->
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <form>
            @csrf
            <input type="hidden" name="survey-id" value="{{ $publish_survey->id }}">
            <input type="hidden" name="selected-service-id" value="">
            <input type="hidden" name="selected-digital-platform-service-id" value="">
                    ...
                    @foreach ($services as $service)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td
                                ...
                                data-digital-platform-service-id="{{ $service->digitalPlatforms->first()?->pivot->id }}"
                            >
                            <x-base.form-check.input 
                                type="checkbox" 
                                {{-- @if($publish_survey->selected_service_id == $service->id) 
                                    checked 
                                @endif --}}
                                {{-- :checked="$service->id == $publish_survey->$service_id" --}}
                            />
                        </x-base.table.td>
            ...
            <div class="mt-6 mb-6 flex flex-col justify-end gap-2 md:flex-row">
                <x-base.button type="button" as="a" href="{{ route('publish-surveys.selectApp', $publish_survey->id) }}" class="w-full border-slate-300 py-3 text-slate-500 dark:border-darkmode-400 md:w-52">
                    Previous
                </x-base.button>
    ...
    <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
        ...
        <form method="GET" action="{{ route('publish-surveys.selectService', $publish_survey->id) }}" class="mt-3 w-20 sm:mt-0">
...
@pushOnce('scripts')
    @vite('resources/js/pages/selectService.js')
@endPushOnce