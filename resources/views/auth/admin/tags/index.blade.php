...
@section('subcontent')
    <h2 class="intro-y mt-10 text-lg font-medium">Tags List</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            <x-base.button class="mr-2 shadow-md" variant="primary" href="{{ route('tags.create') }}" as="a">
                Add New Tag
            </x-base.button>
                        ...
                        <x-base.table.tr class="intro-x">
                            ...
                            <x-base.table.td @class([
                                ...
                                <div class="flex items-center justify-center">
                                    @can('update users')
                                        <a class="mr-3 flex items-center" href="{{ url('tags/'.$tag->id.'/edit') }}">
                                            <x-base.lucide class="mr-1 h-4 w-4" icon="CheckSquare" />
                                            Edit
                                        </a>                                        
                                    @endcan
                                    @can('delete users')
                                        <a class="mr-3 flex items-center text-danger" href="{{ url('tags/'.$tag->id.'/delete') }}">
        ...
        <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
            ...
            <x-base.pagination class="w-full sm:mr-auto sm:w-auto">
                ...
                @else
                    <x-base.pagination.link href="{{ $tags->url(1) }}">
                        <x-base.lucide class="h-4 w-4" icon="ChevronsLeft" />
                    </x-base.pagination.link>
                @endif
                ...
                @else
                    <x-base.pagination.link href="{{ $tags->previousPageUrl() }}">
                        <x-base.lucide class="h-4 w-4" icon="ChevronLeft" />
                    </x-base.pagination.link>
                @endif
            
                <!-- Page Numbers -->
                @foreach ($tags->getUrlRange(max(1, $tags->currentPage() - 1), min($tags->lastPage(), $tags->currentPage() + 1)) as $page => $url)
                ...
                @if ($tags->hasMorePages())
                    <x-base.pagination.link href="{{ $tags->nextPageUrl() }}">
                ...
                <!-- Last Page Link -->
                @if ($tags->hasMorePages())
                    <x-base.pagination.link href="{{ $tags->url($tags->lastPage()) }}">
            ...
            <form method="GET" action="{{ route('tags.index') }}" class="mt-3 w-20 sm:mt-0">
